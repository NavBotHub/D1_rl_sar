...
void RL_Sim::RobotControl()
{
    ...
    // for (int i = 0; i < this->params.Get<int>("num_of_dofs"); ++i)
    // {
    //     std::cout << "Joint " << i << ": " << this->robot_state.motor_state.q[i] << std::endl;
    // }
    if (this->csv_ofs.is_open())
    {
...
void RL_Sim::GetSysJoystick()
{
    ...
    if (has_input)
    {
        // Limit backward motion to -0.6: backward motion only covers down to ~-0.77 (0p70 bin), and training max_backward_curriculum=0.6.
        // Full backward stick deflection (ly=-1.0) exceeds the training/motion range, and policy extrapolation causes crouching with knees touching the ground.
