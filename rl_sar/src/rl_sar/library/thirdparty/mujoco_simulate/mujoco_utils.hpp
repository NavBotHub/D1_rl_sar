...
void PhysicsThread(mj::Simulate* sim, const char* filename) {
  ...
  //
  // 必须加锁删除，并且先把指针置空再解锁：否则主线程的 RenderLoop 可能
  // 还在 Sync()/其他地方访问 m/d（那些地方会加锁），就会出现 UAF。
  // 置空后，别的线程拿到锁如果检查 m/d，会看到 nullptr 而不再访问。
  {
    const std::unique_lock<std::recursive_mutex> lock(sim->mtx);
    mjData* d_local = d;
    mjModel* m_local = m;
    d = nullptr;
    m = nullptr;
    if (d_local) mj_deleteData(d_local);
    if (m_local) mj_deleteModel(m_local);
  }
}
