    ...
    bool use_smooth_curve)
{
    ...
    // cosine ease-in-out: zero velocity at both ends, smooth acceleration in between
    float blend = use_smooth_curve
        ? 0.5f * (1.0f - std::cos(static_cast<float>(M_PI) * percent))
        : percent;
    ...
    {
        fsm_command->motor_command.q[i] = (1.0f - blend) * start_pos[i] + blend * target_pos[i];
