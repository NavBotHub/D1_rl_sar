...
Compatibility note: the canonical documentation path is `~/project/D1_rl_sar`. If an existing machine still uses `~/project/d1_rl_sar`, either keep using that actual path in local commands or rename/re-clone the repository to `~/project/D1_rl_sar`.

### 4.2 Device Tree for the DaMiao Carrier Board

Skip this step for the official Jetson Orin Nano developer kit. For the DaMiao third-party carrier board, install the repository DTB after cloning this repository and before using the onboard LoRa DOG_CTRL serial path. The installer always uses:
...
### 4.4 Download Inference Runtime
...
![D1 Nano wiring diagram](docs/images/d1_nano_wiring_en.png)
...
After updating service templates, rebuild first so the templates under `install/share/rl_sar/systemd/` are refreshed, then run the installer. The installer writes the explicit workspace path passed with `--root` to `/etc/default/rl-sar`, so run it from the actual `rl_sar` workspace on this machine:
...
- If logs show `cd: ... No such file or directory`, `RL_SAR_ROOT` in `/etc/default/rl-sar` is wrong. Run `cat /etc/default/rl-sar`; `RL_SAR_ROOT` should point to the real path returned by `pwd` in the current `rl_sar` workspace, for example `$HOME/project/D1_rl_sar/rl_sar`.
